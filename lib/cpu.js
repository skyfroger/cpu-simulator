...
var cpu = {
	...
	step: function() {
		...
		switch(cpu.state) {
			...
			case 8: //The *код операции (opcode)* 0001 means add the value in the *Аккумулятор (ACC)* register to the data stored in memory at the address specified by the *операнд (operand)*"
				setState(81, "Декодирование", "*Блок декодирования* отправляет *операнд (operand)* в *Регистр адреса памяти (MAR)* который также копируется в *Адресную шину*");
			...
			case 9: //"The *код операции (opcode)* 0010 means subtract the value stored in memory at the address specified by the *операнд (operand)* from the value in the *Аккумулятор (ACC)* register"
				setState(91, "Декодирование", "*Блок декодирования* отправляет *операнд (operand)* в *Регистр адреса памяти (MAR)* значение которого также копируется в *Адресную шину*");
				cpu.registers.mar = cpu.registers.cir & 0x0F;
				cpu.updateValues();
				$('.active').removeClass('active');
	...
	init: function(jqCPU) {
		...
		function getRegisterHtml(name, value, desc) {
			return '<div class="register" id="reg_' + name.toLowerCase()+'"><div class="reg_name" title="' + desc +'">' + name + '</div><div id="reg_' + name.toLowerCase() + '_val" class="reg_val value value_denary editable" data-description="' + desc + '">' + value + '</div></div>';
		}
		html += getRegisterHtml('PC', 0, "");
		html += getRegisterHtml('MAR', 0, "");
		html += getRegisterHtml('MDR', 0, "");
		html += getRegisterHtml('ACC', 0, "");
		html += getRegisterHtml('CIR', 0, "");
		...
		html += '<tr class="decode_row_6"><td>0110</td><td>адрес</td><td>Безусловный переход</td></tr>';
...
$(function() {
	...
	tippy('#reg_pc', {
		content: "<strong>Счётчик команд (Program Counter)</strong> всегда содержит адрес инструкции, которая будет выполняться следующей",
		allowHTML: true,
		delay: [500,0],
	});

	tippy('#reg_mar', {
		content: "<strong>Регистр адреса памяти (Memory Address Register)</strong> хранит в себе либо адрес, из которого процессору необходимо извлечь данные, либо адрес, по которому данные будут отправлены и сохранены.",
		allowHTML: true,
		delay: [500,0],
	});

	tippy('#reg_mdr', {
		content: "<strong>Регистр данных (Memory Data Register)</strong> временно хранит данные полученные из ячейки памяти с адресом, хранящимся в <strong>регистре адреса памяти (MAR)</strong>.",
		allowHTML: true,
		delay: [500,0],
	});
	...
	tippy('#reg_cir', {
		onShow(instance){
			let mainText = "<p><strong>Регистр команд (Instruction Register)</strong> хранит инструкцию, которую в данный момент должен выполнить процессор. Команда, записанная в виде двоичного числа, определяет, какие команды <strong>Блок управления</strong> будет давать процессору.</p>"
			const regValue = $('#reg_cir_val').html();

			if (regValue.length == 8) {
				const opCode = regValue.slice(0, 4);
				const operandVal = regValue.slice(4);
				let additionalText = "<p><strong>Текущий код операции:</strong> " + opCode + "</p>";
				additionalText += "<p><strong>Операнд:</strong> " + operandVal + "</p>";
				mainText += additionalText;
			}
			
			instance.setContent(mainText);
		},
		allowHTML: true,
		delay: [500,0],
	});
	...
	tippy('#alu', {
		content: "<strong>Арифметико-логическое устройство</strong> выполняет арифметические и логические операции над операндами.",
		allowHTML: true,
		delay: [1000,0],
	});

	tippy('#cu', {
		content: "<strong>Блок управления (Control Unit)</strong> считывает значение в регистре команд и декодирует его, чтобы дать необходимые команды другим блокам процессора.",
		allowHTML: true,
		delay: [1000,0],
	});

	tippy('#reg_acc', {
		content: "<strong>Регистр-аккумулятор (ACC)</strong> предназначен для временного хранения промежуточных результатов арифметических и логических операций.",
		allowHTML: true,
		delay: [500,0],
	});
});