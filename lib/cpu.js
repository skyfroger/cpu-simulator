...
var cpu = {
	...
	step: function() {
		...
		switch(cpu.state) {
			...
			case 0:
				setState(1, "Выборка", "The *Control Unit* copies the value in the *Program Counter* register to the *Memory Address Register* and onto the *Address Bus*");
				...
				break;
			case 1:
				setState(2, "Выборка", "The *Control Unit* tells the memory store to look at the address on the *Address Bus* and load the value stored there onto the *Data Bus*");
				$('.active').removeClass('active');
				$('#ram_value_' + cpu.registers.mar).addClass('active');
			break;
			
			case 2:
				setState(3, "Выборка", "The *Control Unit* stores the value on the *Data Bus* into the *Memory Data Register*");
			...
			case 3:
				setState(4, "Выборка", "The *Control Unit* copies the value from the *Memory Data Register* into the *Current Instruction Register*");
			...
			case 4:
				setState(5,"Выборка", "The *Control Unit* increments the *Program Counter*");
				cpu.registers.pc++;
				cpu.updateValues();
				$('.active').removeClass('active');
			...
			case 5:
				setState(6, "Декодирование", "The *Decode Unit* breaks the value in the *Current Instruction Register* into the *opcode* and *operand*."); 
				$('.active').removeClass('active');
			...
			case 6:
				...
				switch(opcode) {
					case 0:
						setState(7, "Декодирование", "The *opcode* 0000 means end the program");
					break;
					
					case 1:
						setState(8, "Декодирование", "The *opcode* 0001 means add the value in the *Accumulator* register to the data stored in memory at the address specified by the *operand*");
					break;
					
					case 2:
						setState(9, "Декодирование", "The *opcode* 0010 means subtract the value stored in memory at the address specified by the *operand* from the value in the *Accumulator* register");
					break;
					
					case 3:
						setState(10, "Декодирование", "The *opcode* 0011 means store the value in the *Accumulator* register into memory at the address specified by the *operand*");
					break;
					
					case 5:
						setState(11, "Декодирование", "The *opcode* 0101 means load the value from memory (at the address specified by the *operand*) into the *Accumulator* register");
					break;
					
					case 6:
						setState(12, "Декодирование", "The *opcode* 0110 means branch (unconditionally)");
					break;
					
					case 7:
						setState(13, "Декодирование", "The *opcode* 0111 means branch if the *Accumulator* stores a value equal to 0");
					break;
					
					case 8:
						setState(14, "Декодирование", "The *opcode* 1000 means branch if the *Accumulator* stores a value greater or equal to 0 (not negative)");
					break;
					
					case 9:
						setState(15, "Декодирование", "The *opcode* 1001 means either input or output, depending on the *operand*");
					break;
			...
			case 7:
				setState(7, "Выполнение", "The CPU has halted so the *Control Unit* doesn't fetch any more instructions");
				$('.active').removeClass('active');
				cpu.running = false;
				break;
			...
			case 8: //The *opcode* 0001 means add the value in the *Accumulator* register to the data stored in memory at the address specified by the *operand*"
				setState(81, "Декодирование", "The *Decode Unit* sends the *opcode* to the *Memory Address Register* which gets copied onto the *Address Bus*");
			...
			case 81: 
				setState(82, "Выполнение", "The *Control Unit* tells the memory store to look at the address on the *Address Bus* and place that value on the *Data Bus*");
				$('.active').removeClass('active');
				$('#ram_value_' + cpu.registers.mar).addClass('active');
				break;
			case 82:
				setState(83, "Выполнение", "The *Control Unit* copies the value on the *Data Bus* into the *Memory Data Register*");
				cpu.registers.mdr = cpu.ram[cpu.registers.mar];
				cpu.updateValues();
				$('.active').removeClass('active');
				$('#reg_mdr').addClass('active');
				break;
			case 83:
				setState(0, "Выполнение", "The *opcode* and *Control Unit* signals the *Arithmetic Logic Unit* to add the values stored in the *Accumulator* and *Memory Data Register*s. The result gets saved back in the *Accumulator* register.");
			...
			case 9: //"The *opcode* 0010 means subtract the value stored in memory at the address specified by the *operand* from the value in the *Accumulator* register"
				setState(91, "Декодирование", "The *Decode Unit* sends the *opcode* to the *Memory Address Register* which gets copied onto the *Address Bus*");
				cpu.registers.mar = cpu.registers.cir & 0x0F;
				cpu.updateValues();
				$('.active').removeClass('active');
			...
			case 91: 
				setState(92, "Выполнение", "The *Control Unit* tells the memory store to look at the address on the *Address Bus* and place that value on the *Data Bus*");
				$('.active').removeClass('active');
				$('#ram_value_' + cpu.registers.mar).addClass('active');
				break;
			case 92:
				setState(93, "Выполнение", "The *Control Unit* copies the value on the *Data Bus* into the *Memory Data Register*");
				$('.active').removeClass('active');
				$('#reg_mdr').addClass('active');
				cpu.registers.mdr = cpu.ram[cpu.registers.mar];
				cpu.updateValues();
				break;
			case 93:
				setState(0, "Выполнение", "The *opcode* and *Control Unit* signals the *Arithmetic Logic Unit* to subtract the values stored in the *Memory Data Register* from the value stored in the *Accumulator* register. The result gets saved back in the *Accumulator* register.");
			...
			case 10: //"The *opcode* 0011 means store the value in the *Accumulator* register into memory at the address specified by the *operand*"
				setState(101, "Декодирование", "The *opcode* and *Control Unit* sends the value stored in the *Accumulator* register to the *Memory Data Register* which gets copied on to the *Data Bus*");
				...
				break;
			case 101:
				setState(102, "Декодирование", "The *Decode Unit* sends the *operand* to the *Memory Address Register* which gets copied onto the *Address Bus*");
				cpu.registers.mar = cpu.registers.cir & 0x0F;
				cpu.updateValues();
				$('.active').removeClass('active');
				$('#reg_mar').addClass('active');
			break;
			case 102: 
				setState(0, "Выполнение", "The *Control Unit* tells the memory store to store the value on the *Data Bus* into the address on the *Address Bus*");
			...
			case 11: //"The *opcode* 0101 means load the value from memory (at the address specified by the *operand*) into the *Accumulator* register"
				setState(111, "Декодирование", "The *Decode Unit* sends the *operand* to the *Memory Address Register* which gets copied onto the *Address Bus*");
				cpu.registers.mar = cpu.registers.cir & 0x0F;
				cpu.updateValues();
				$('.active').removeClass('active');
				$('#reg_mar').addClass('active');
			...
			case 111: 
				setState(112, "Выполнение", "The *Control Unit* tells the memory store to look at the address on the *Address Bus* and place that value on the *Data Bus*");
				$('.active').removeClass('active');
				$('#ram_value_' + cpu.registers.mar).addClass('active');
				break;
			case 112:
				setState(113, "Выполнение", "The *Control Unit* copies the value on the *Data Bus* into the *Memory Data Register*");
				cpu.registers.mdr = cpu.ram[cpu.registers.mar];
				cpu.updateValues();
				$('.active').removeClass('active');
				$('#reg_mdr').addClass('active');
				break;
			case 113:
				setState(20, "Выполнение", "The *opcode* and *Control Unit* sends the value in the *Memory Data Register* to the *Accumulator* register.");
				cpu.registers.acc = cpu.registers.mdr;
				cpu.updateValues();
				$('.active').removeClass('active');
			...
			case 12://The *opcode* 0110 means branch (unconditionally)
				setState(20, "Выполнение", "The *operand* gets stored in the *Program Counter*");
			...
			case 13://The *opcode* 0111 means branch if the *Accumulator* stores a value equal to 0
				setState(20, "Выполнение", "The *Control Unit* and *opcode* makes the *Arithmetic Logic Unit* check to see if the *Accumulator* register contains a zero. If it does, the *operand* gets copied into the *Program Counter* register");
			...
			case 14://The *opcode* 1000 means branch if the *Accumulator* stores a value greater than or equal to 0"
				setState(20, "Выполнение", "The *Control Unit* and *opcode* makes the *Arithmetic Logic Unit* check to see if the *Accumulator* register contains a value greater than zero. If it does, the *operand* gets copied into the *Program Counter* register");
				if(cpu.registers.acc >= 0)
					cpu.registers.pc = cpu.registers.cir & 0x0F;
				$('.active').removeClass('active');
				cpu.updateValues();
			...
			case 15://The *opcode* 1001 means either input or output, depending on the *operand*"
				if((cpu.registers.cir & 0x0F) == 1) {
					setState(20, "Выполнение", "The *opcode* 0001 and the *Control Bus* reads from the input device and places the input value into the *Accumulator* register");
				...
				if((cpu.registers.cir & 0x0F) == 2) {
					setState(20, "Выполнение", "The *opcode* 0010 and the *Control Bus* causes the value of the *Accumulator* register to be sent to the output device");
			...
			case 20:
				setState(0, "Выполнение", "The *Control Unit* checks for interrupts and either branches to the relevant interrupt service routine or starts the cycle again.");
				break;
