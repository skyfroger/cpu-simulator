...
var cpu = {
	...
	step: function() {
		...
		switch(cpu.state) {
			...
			case 7:
				setState(7, "Выполнение", "The CPU has halted so the *Блок управления (CU)* doesn't fetch any more instructions");
				$('.active').removeClass('active');
				cpu.running = false;
				break;
			...
			case 8: //The *код операции (opcode)* 0001 means add the value in the *Аккумулятор (ACC)* register to the data stored in memory at the address specified by the *операнд (operand)*"
				setState(81, "Декодирование", "The *Блок декодирования* sends the *код операции (opcode)* to the *Регистр адреса памяти (MAR)* which gets copied onto the *Адресная шина*");
			...
			case 81: 
				setState(82, "Выполнение", "The *Блок управления (CU)* tells the memory store to look at the address on the *Адресная шина* and place that value on the *Шина данных*");
				$('.active').removeClass('active');
				$('#ram_value_' + cpu.registers.mar).addClass('active');
				break;
			case 82:
				setState(83, "Выполнение", "The *Блок управления (CU)* copies the value on the *Шина данных* into the *Регистр данных (MDR)*");
				cpu.registers.mdr = cpu.ram[cpu.registers.mar];
				cpu.updateValues();
				$('.active').removeClass('active');
				$('#reg_mdr').addClass('active');
				break;
			case 83:
				setState(0, "Выполнение", "The *код операции (opcode)* and *Блок управления (CU)* signals the *Arithmetic Logic Unit* to add the values stored in the *Аккумулятор (ACC)* and *Регистр данных (MDR)*s. The result gets saved back in the *Аккумулятор (ACC)* register.");
			...
			case 9: //"The *код операции (opcode)* 0010 means subtract the value stored in memory at the address specified by the *операнд (operand)* from the value in the *Аккумулятор (ACC)* register"
				setState(91, "Декодирование", "The *Блок декодирования* sends the *код операции (opcode)* to the *Регистр адреса памяти (MAR)* which gets copied onto the *Адресная шина*");
				cpu.registers.mar = cpu.registers.cir & 0x0F;
				cpu.updateValues();
				$('.active').removeClass('active');
			...
			case 91: 
				setState(92, "Выполнение", "The *Блок управления (CU)* tells the memory store to look at the address on the *Адресная шина* and place that value on the *Шина данных*");
				$('.active').removeClass('active');
				$('#ram_value_' + cpu.registers.mar).addClass('active');
				break;
			case 92:
				setState(93, "Выполнение", "The *Блок управления (CU)* copies the value on the *Шина данных* into the *Регистр данных (MDR)*");
				$('.active').removeClass('active');
				$('#reg_mdr').addClass('active');
				cpu.registers.mdr = cpu.ram[cpu.registers.mar];
				cpu.updateValues();
				break;
			case 93:
				setState(0, "Выполнение", "The *код операции (opcode)* and *Блок управления (CU)* signals the *Arithmetic Logic Unit* to subtract the values stored in the *Регистр данных (MDR)* from the value stored in the *Аккумулятор (ACC)* register. The result gets saved back in the *Аккумулятор (ACC)* register.");
			...
			case 10: //"The *код операции (opcode)* 0011 means store the value in the *Аккумулятор (ACC)* register into memory at the address specified by the *операнд (operand)*"
				setState(101, "Декодирование", "The *код операции (opcode)* and *Блок управления (CU)* sends the value stored in the *Аккумулятор (ACC)* register to the *Регистр данных (MDR)* which gets copied on to the *Шина данных*");
				...
				break;
			case 101:
				setState(102, "Декодирование", "The *Блок декодирования* sends the *операнд (operand)* to the *Регистр адреса памяти (MAR)* which gets copied onto the *Адресная шина*");
				cpu.registers.mar = cpu.registers.cir & 0x0F;
				cpu.updateValues();
				$('.active').removeClass('active');
				$('#reg_mar').addClass('active');
			break;
			case 102: 
				setState(0, "Выполнение", "The *Блок управления (CU)* tells the memory store to store the value on the *Шина данных* into the address on the *Адресная шина*");
			...
			case 11: //"The *код операции (opcode)* 0101 means load the value from memory (at the address specified by the *операнд (operand)*) into the *Аккумулятор (ACC)* register"
				setState(111, "Декодирование", "The *Блок декодирования* sends the *операнд (operand)* to the *Регистр адреса памяти (MAR)* which gets copied onto the *Адресная шина*");
				cpu.registers.mar = cpu.registers.cir & 0x0F;
				cpu.updateValues();
				$('.active').removeClass('active');
				$('#reg_mar').addClass('active');
			...
			case 111: 
				setState(112, "Выполнение", "The *Блок управления (CU)* tells the memory store to look at the address on the *Адресная шина* and place that value on the *Шина данных*");
				$('.active').removeClass('active');
				$('#ram_value_' + cpu.registers.mar).addClass('active');
				break;
			case 112:
				setState(113, "Выполнение", "The *Блок управления (CU)* copies the value on the *Шина данных* into the *Регистр данных (MDR)*");
				cpu.registers.mdr = cpu.ram[cpu.registers.mar];
				cpu.updateValues();
				$('.active').removeClass('active');
				$('#reg_mdr').addClass('active');
				break;
			case 113:
				setState(20, "Выполнение", "The *код операции (opcode)* and *Блок управления (CU)* sends the value in the *Регистр данных (MDR)* to the *Аккумулятор (ACC)* register.");
				cpu.registers.acc = cpu.registers.mdr;
				cpu.updateValues();
				$('.active').removeClass('active');
			...
			case 12://The *код операции (opcode)* 0110 means branch (unconditionally)
				setState(20, "Выполнение", "The *операнд (operand)* gets stored in the *Счетчик команд (PC)*");
			...
			case 13://The *код операции (opcode)* 0111 means branch if the *Аккумулятор (ACC)* stores a value equal to 0
				setState(20, "Выполнение", "The *Блок управления (CU)* and *код операции (opcode)* makes the *Arithmetic Logic Unit* check to see if the *Аккумулятор (ACC)* register contains a zero. If it does, the *операнд (operand)* gets copied into the *Счетчик команд (PC)* register");
			...
			case 14://The *код операции (opcode)* 1000 means branch if the *Аккумулятор (ACC)* stores a value greater than or equal to 0"
				setState(20, "Выполнение", "The *Блок управления (CU)* and *код операции (opcode)* makes the *Arithmetic Logic Unit* check to see if the *Аккумулятор (ACC)* register contains a value greater than zero. If it does, the *операнд (operand)* gets copied into the *Счетчик команд (PC)* register");
				if(cpu.registers.acc >= 0)
					cpu.registers.pc = cpu.registers.cir & 0x0F;
				$('.active').removeClass('active');
				cpu.updateValues();
			...
			case 15://The *код операции (opcode)* 1001 means either input or output, depending on the *операнд (operand)*"
				if((cpu.registers.cir & 0x0F) == 1) {
					setState(20, "Выполнение", "The *код операции (opcode)* 0001 and the *Control Bus* reads from the input device and places the input value into the *Аккумулятор (ACC)* register");
				...
				if((cpu.registers.cir & 0x0F) == 2) {
					setState(20, "Выполнение", "The *код операции (opcode)* 0010 and the *Control Bus* causes the value of the *Аккумулятор (ACC)* register to be sent to the output device");
			...
			case 20:
				setState(0, "Выполнение", "The *Блок управления (CU)* checks for interrupts and either branches to the relevant interrupt service routine or starts the cycle again.");
				break;
